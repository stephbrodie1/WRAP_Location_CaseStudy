...
predict_glmm <- function(model) {
  dummy = data.frame(year = unique(dat_all$year),
                     ...
                     z_s = dat_all$z_s[1])
  pred = predict(model,
                 newdata=rbind(dat_x[,c("year","lat","lon","temp_s","chla_s", "mld_s","z_s")],
...
for (yy in c(2000, 2040, 2080)) {
  ...
  abundx <- predict(gam_E_N, dat_x, type="response")
...
for (yy in c(2000, 2040, 2080)) {
  ...
  abundx <- predict(brt_E_N, dat_x, type="response", n.trees=brt_E_N$gbm.call$best.trees)
  dat_x$pred_x <- presx * abundx
  r_pred <- rasterFromXYZ(dat_x[,c("lon","lat","pred_x")])
  ...
  abundx <- predict(brt_ES_N, dat_x, type="response", n.trees=brt_ES_N$gbm.call$best.trees)
  dat_x$pred_x <- presx * abundx
  r_pred <- rasterFromXYZ(dat_x[,c("lon","lat","pred_x")])
  ...
  abundx <- predict(brt_EST_N, dat_x, type="response", n.trees=brt_EST_N$gbm.call$best.trees)
  dat_x$pred_x <- presx * abundx
  r_pred <- rasterFromXYZ(dat_x[,c("lon","lat","pred_x")])
